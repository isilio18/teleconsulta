...
DcmViewer.prototype.showSeries = function(files) {
    ...
    async function getFiles(url) {
        var files = new Array()
        var entries  = await (new zip.ZipReader(new zip.HttpReader("https://teleconsulta.gobeltech.com/proceso/documento/ver/"+url))).getEntries();
        ...
        if (!entry.directory) {            
                ...
                if(entry.filename.includes(".bmp") || entry.filename.includes(".BMP")) {
                    contentType = "image/bmp";
                }
                if(entry.filename.includes(".dcm") || entry.filename.includes(".DCM")) {
                    contentType = "application/dicom";
                }

                var file = new File([data], entry.filename, {type: contentType, lastModified: Date.now()});             
                files.push(file)
                console.log(file)
...
/**
 *  Updates the progress bar, parses the files and renders it on a tree. Only Dicom files are processed.
 * @param {Event} e Change-event of the input element.
 */
DcmViewer.prototype.urlHandler = async function(url){
