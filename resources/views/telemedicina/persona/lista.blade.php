...
@section('js_after')
    ...
    <script>
        $('#borrar').on('show.bs.modal', function (event) {
            $("#borrarForm").attr('action','{{ url('/administracion/banco/eliminar') }}');
...
<div class="content content-full content-boxed">
    ...
    <div class="block block-rounded block-bordered">
        ...
        <form action="{{ url('/telemedicina/persona/lista') }}" method="get">
            <div class="row">
                ...
                <div class="col-sm-12 col-md-6">
                    <div class="input-group">
                        <input type="text" class="form-control" id="q" name="q" value="{{ $q }}" placeholder="Buscar cedula...">
            ...
            <table class="table table-bordered table-striped table-vcenter">
                ...
                @foreach($tab_persona as $key => $value)
                    <tr>
                        ...
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->cedula }}</em></td>
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->nombres }} {{ $value->apellidos }}</em></td>
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->telefono }}</em></td>
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->direccion }}</em></td>
                        <td class="text-center">                              
                                <a href="{{ url('/telemedicina/persona/editar').'/'. $value->id }}">
            ...
            {{ $tab_persona->appends(Request::only(['perPage','q']))->render() }}         
